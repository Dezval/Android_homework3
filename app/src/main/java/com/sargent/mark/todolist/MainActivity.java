...
public class MainActivity extends AppCompatActivity implements AddToDoFragment.OnDialogCloseListener, UpdateToDoFragment.OnUpdateDialogCloseListener{
    ...
    protected void onStart() {
        ...
        }, new ToDoListAdapter.CheckBoxListener(){ // added this parameter for checkbox status

            @Override
            public void onCheckedBox(boolean checked, long id) {
                Log.d(TAG, "CHECKING FOR CHECKBOX CHANGE");
                updateCheck(db, checked ? "true":"false", id);
            }
        });
    ...
    private long addToDo(SQLiteDatabase db, String description, String duedate) {
        ...
        cv.put(Contract.TABLE_TODO.COMPLETE, "false");
    ...
    private boolean isChecked(long id){
        CheckBox check_box = (CheckBox) findViewById(R.id.itemCheckbox);
        return check_box.isPressed();
    }
    ...
    private int updateCheck(SQLiteDatabase db, String completed, long id){
        ContentValues cv = new ContentValues();
        Log.d(TAG,"completed: " + completed);
        cv.put(Contract.TABLE_TODO.COMPLETE, completed);
